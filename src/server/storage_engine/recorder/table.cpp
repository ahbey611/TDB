...
/**
 * 创建索引，然后遍历所有数据，插入索引, 最后将索引放到表的元数据中,
 * 并且将元数据写入文件
 * @param trx 事务
...
RC Table::create_index(Trx *trx,
  ...
  if (rc != RC::SUCCESS) {
    LOG_INFO(
        "Failed to init IndexMeta in table:%s, index_name:%s, field_amount:%d",
  ...
  while (scanner.has_next()) {
    ...
    if (rc != RC::SUCCESS) {
      LOG_WARN(
          "failed to scan records while creating index. table=%s, index=%s, "
          "rc=%s",
          name(), index_name, strrc(rc));
      return rc;
    }
  ...
  scanner.close_scan();
  LOG_INFO("inserted all records into new index. table=%s, index=%s", name(),
           index_name);
  ...
  LOG_INFO("Successfully added a new index (%s) on the table (%s)", index_name,
           name());
  return rc;
}
