...
RC IndexScanPhysicalOperator::open(Trx *trx) {
  ...
  IndexScanner *index_scanner = index_->create_scanner(
      left_key, left_value_.length(), left_inclusive_, right_key,
      right_value_.length(), right_inclusive_);
  ...
  if (table_alias_.empty()) {
    ...
    LOG_WARN(
        ...
        "Hint: Consider calling set_table_alias() on IndexScanOperator to set "
